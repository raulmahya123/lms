...
// =====================
// Auth scaffolding (Breeze/Fortify/Jetstream, dsb.)
// =====================
...
// =====================
// TEST IQ — hanya auth (TANPA verified)
// supaya user baru (belum verifikasi email) bisa langsung Tes IQ
// =====================
Route::middleware(['auth'])->group(function () {
    Route::get('/iq/{testIq}',         [UserTestIqController::class, 'show'])->name('admin.test-iq.show');
    Route::post('/iq/{testIq}/submit', [UserTestIqController::class, 'submit'])->name('admin.test-iq.submit');
    Route::get('/iq/{testIq}/result',  [UserTestIqController::class, 'result'])->name('admin.test-iq.result');
});

// =====================
// User Area (login + email verified diperlukan)
// =====================
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // daftar paket membership versi user
    Route::get('/memberships/plans', [UserMembershipController::class, 'plans'])->name('app.memberships.plans');

    // mulai subscribe (buat membership status "pending" untuk plan terpilih)
    Route::post('/memberships/subscribe/{plan}', [UserMembershipController::class, 'subscribe'])->name('app.memberships.subscribe');

    // halaman checkout membership pending
    Route::get('/memberships/checkout/{membership}', [UserMembershipController::class, 'checkout'])->name('app.memberships.checkout');

    // aktivasi membership
    Route::post('/memberships/activate/{membership}', [UserMembershipController::class, 'activate'])->name('app.memberships.activate');

    // batalkan/nonaktifkan membership user
    Route::post('/memberships/cancel/{membership}', [UserMembershipController::class, 'cancel'])->name('app.memberships.cancel');

    // update membership oleh user (opsional)
    Route::patch('/memberships/{membership}', [UserMembershipController::class, 'update'])->name('app.memberships.update');
    ...
    // Request akses Drive resource lesson
    ...
    Route::get('/certificates', [CertificateController::class, 'index'])->name('app.certificates.index');

    // preview/show/download issue
    Route::get('/certificates/{issue}', [CertificateController::class, 'show'])->name('app.certificates.show');
    Route::get('/certificates/{issue}/preview', [CertificateController::class, 'preview'])->name('app.certificates.preview');
    Route::get('/certificates/{issue}/download', [CertificateController::class, 'download'])->name('app.certificates.download');
    ...
    // =====================
    // PSY-TESTS (skala/kepribadian) — versi USER
    // =====================
    Route::prefix('psy-tests')->group(function () {
        ...
        Route::get('/{slugOrId}/questions/{question}', [UserPsyQuestionController::class, 'show'])
            ->whereNumber('question')->name('app.psytests.questions.show');

        // Mulai / lanjut attempt
        Route::post('/{slugOrId}/start', [UserPsyAttemptController::class, 'start'])
            ->middleware('throttle:quiz')->name('app.psy.attempts.start');

        // Simpan jawaban 1 soal
        Route::post('/{slugOrId}/q/{question}/answer', [UserPsyAttemptController::class, 'answer'])
            ->middleware('throttle:quiz')->whereNumber('question')->name('app.psy.attempts.answer');

        // Submit & hitung hasil
        Route::get('/{slugOrId}/submit', [UserPsyAttemptController::class, 'submit'])->name('app.psy.attempts.submit');

        // Lihat hasil attempt
        Route::get('/{slugOrId}/result/{attempt}', [UserPsyAttemptController::class, 'result'])->name('app.psy.attempts.result');
    });
    ...
    Route::post('qa-threads/{thread}/replies', [UserQaThreadController::class, 'store'])->name('app.qa-threads.replies.store');
    Route::patch('qa-replies/{reply}/answer', [UserQaThreadController::class, 'markAnswer'])->name('app.qa-replies.answer');
    Route::delete('qa-replies/{reply}', [UserQaThreadController::class, 'destroy'])->name('app.qa-replies.destroy');
    ...
    Route::get('/profile/updateinformation', function (Request $r) {
        return view('profile.updateinformation', [
            'user'           => $r->user(),
            'mustVerifyEmail'=> $r->user() instanceof MustVerifyEmail,
    ...
    Route::get('/profile/delacc', fn () => view('profile.delacc'))->name('profile.delete.confirm');
...
Route::middleware(['auth', 'can:admin'])
    ...
    ->group(function () {
        ...
        Route::get('psy-questions', [\App\Http\Controllers\Admin\PsyQuestionController::class, 'globalIndex'])->name('psy-questions.index');
        Route::get('psy-questions/create', [\App\Http\Controllers\Admin\PsyQuestionController::class, 'globalCreate'])->name('psy-questions.create');
        Route::post('psy-questions', [\App\Http\Controllers\Admin\PsyQuestionController::class, 'globalStore'])->name('psy-questions.store');

        // Attempts psy-test (admin)
        Route::resource('psy-attempts', \App\Http\Controllers\Admin\PsyAttemptController::class)->only(['index', 'show', 'destroy']);
    });
