...
<div class="max-w-4xl mx-auto space-y-8 pb-12">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-text-main">Perbarui Diskusi</h1>
    </div>
    <div class="shrink-0 flex gap-3">
      <a href="{{ route('app.qa-threads.show',$thread) }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white border-2 border-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-colors">
  ...
  <form method="POST" action="{{ route('app.qa-threads.update',$thread) }}" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
    @csrf @method('PUT')
    ...
    <div class="p-8 space-y-6">
      ...
      <div>
        ...
        <input type="text" name="title" value="{{ old('title',$thread->title) }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-bold text-text-main placeholder-gray-400" required autofocus>
      ...
      <div>
        ...
        <textarea name="body" rows="8" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all placeholder-gray-400 resize-y leading-relaxed" required>{{ old('body',$thread->body) }}</textarea>
    ...
    <div class="p-6 border-t border-gray-50 bg-gray-50/30 flex items-center justify-end gap-3">
      <a href="{{ route('app.qa-threads.show',$thread) }}" class="px-6 py-3 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors">
        Batal
      </a>
