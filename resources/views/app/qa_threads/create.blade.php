...
<div class="max-w-3xl mx-auto">
  <h1 class="text-2xl font-bold mb-6">Buat Thread</h1>

  <form method="POST" action="{{ route('app.qa-threads.store') }}" class="bg-white rounded-xl shadow p-6 space-y-5">
    @csrf
    ...
    <div>
      ...
      <textarea name="body" rows="8" class="w-full border rounded-lg px-3 py-2" required>{{ old('body') }}</textarea>
      @error('body') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
    </div>

    <div class="flex items-center gap-2">
      <button class="px-4 py-2 rounded-lg bg-blue-600 text-white">Kirim</button>
      <a href="{{ route('app.qa-threads.index') }}" class="px-4 py-2 rounded-lg bg-gray-100">Batal</a>
    </div>
  </form>
</div>
@endsection
