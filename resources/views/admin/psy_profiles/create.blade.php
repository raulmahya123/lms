...
@section('title','Tambah Profil Psikologi — Admin')
...
<div class="max-w-3xl mx-auto space-y-8 pb-12">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Tambah Profil</h1>
      <p class="text-sm text-text-soft mt-1">Buat rentang skor dan interpretasi hasil untuk tes psikologi tertentu.</p>
    </div>
    <div class="shrink-0 flex gap-3">
      <a href="{{ route('admin.psy-profiles.index', ['test_id'=>old('test_id')]) }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-colors">
  ...
  <form method="POST" action="{{ route('admin.psy-profiles.store') }}" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
    @csrf
    ...
    <div class="p-8 space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-gray-50/50 border border-gray-100 rounded-2xl">
          <div>
            ...
            <input type="number" name="min_total" value="{{ old('min_total', 0) }}" min="0"
          ...
          <div>
            ...
            <input type="number" name="max_total" value="{{ old('max_total', 0) }}" min="0"
                   class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-bold text-text-main" required>
    ...
    <div class="p-6 border-t border-gray-50 bg-gray-50/30 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
      <a href="{{ route('admin.psy-profiles.index', ['test_id'=>old('test_id')]) }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors text-center">
        Batal
      </a>
