    ...
    <div>
      <h1 class="text-2xl font-extrabold tracking-wide flex items-center gap-2">
        ...
        <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
          <path d="M4 4h16v12H5.17L4 17.17V4Z"/><path d="M8 8h8v2H8V8Zm0 3h5v2H8v-2Z"/>
        </svg>
        Certificate Issues
      </h1>
      <p class="text-sm opacity-70">Daftar sertifikat terbit beserta serial, user, course/test, skor, dan jenis assessment.</p>
    </div>
          ...
          @forelse($issues as $i)
            <tr class="border-t align-top">
              ...
              <td class="p-3 font-mono">
                <div class="flex items-center gap-2">
                  <span class="truncate max-w-[180px]" title="{{ $i->serial }}">{{ $i->serial }}</span>
                  ...
                  <button type="button" class="p-1 rounded hover:bg-gray-100"
                          ...
                          title="Copy serial">
                    <svg class="w-4 h-4 opacity-70" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5.75A2.75 2.75 0 0 1 10.75 3h6.5A2.75 2.75 0 0 1 20 5.75v6.5A2.75 2.75 0 0 1 17.25 15h-6.5A2.75 2.75 0 0 1 8 12.25v-6.5Zm-2 4A2.75 2.75 0 0 0 3.25 12.5v5.75A2.75 2.75 0 0 0 6 21h5.75A2.75 2.75 0 0 0 14.5 18.25V17H6a2 2 0 0 1-2-2v-5.5Z"/></svg>
                  </button>
                </div>
              </td>
              ...
              <td class="p-3">
                @if($i->assessment_type==='psych')
                  <span class="inline-flex items-center gap-1">
                    {{-- brain icon --}}
                    <svg class="w-4 h-4 opacity-70" viewBox="0 0 24 24" fill="currentColor"><path d="M8.5 6.5A3.5 3.5 0 0 1 12 3a3.5 3.5 0 0 1 3.5 3.5A3.25 3.25 0 0 1 19 9.75 3.25 3.25 0 0 1 15.75 13H9.5A3.5 3.5 0 0 1 8.5 6.5Z"/></svg>
                    Psych Test
                  </span>
                @else
                  <span class="truncate max-w-[320px]" title="{{ $i->course->title ?? '' }}">{{ $i->course->title ?? '—' }}</span>
                @endif
              </td>

              <td class="p-3">
                @php $isCourse = $i->assessment_type === 'course'; @endphp
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs {{ $isCourse ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                  <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    @if($isCourse)
                      <path d="M4.5 6.75A2.25 2.25 0 0 1 6.75 4.5h10.5A2.25 2.25 0 0 1 19.5 6.75V15l-7.5-3-7.5 3V6.75Z"/>
                    @else
                      <path d="M12 2.75a5.25 5.25 0 0 1 5.25 5.25v.5a3.5 3.5 0 1 1 0 7h-1.5a3 3 0 0 1-3 3H9.5A3.5 3.5 0 1 1 9.5 8h.25A5.25 5.25 0 0 1 12 2.75Z"/>
                    @endif
                  </svg>
              ...
              <td class="p-3 text-center">
                <div class="flex items-center justify-center gap-2">
                  ...
                  <form method="POST" action="{{ route('admin.certificate-issues.destroy',$i) }}"
                        onsubmit="return confirm('Delete issue?')" class="inline">
                    @csrf @method('DELETE')
