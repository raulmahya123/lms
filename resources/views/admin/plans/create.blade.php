...
@section('title', 'Buat Paket Baru — Admin')
...
<div class="max-w-4xl mx-auto space-y-8 pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Buat Paket Belajar</h1>
            <p class="text-sm text-text-soft mt-1">Konfigurasi nama, harga, periode, fitur, dan akses course.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.plans.store') }}" class="space-y-8" id="planForm">
        @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" 
                               ...
                               placeholder="Contoh: Paket Premium Bulanan" required>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="period" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-medium text-text-main appearance-none cursor-pointer" required>
                                <option value="monthly" @selected(old('period')==='monthly')>Bulanan (Monthly)</option>
                                <option value="yearly"  @selected(old('period')==='yearly')>Tahunan (Yearly)</option>
                                <option value="lifetime" @selected(old('period')==='lifetime')>Selamanya (Lifetime)</option>
                            </select>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {{-- Features Builder --}}
            <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden" x-data="featureBuilder()">
                <div class="px-8 py-5 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                    <h2 class="text-lg font-extrabold text-text-main flex items-center gap-2">
                ...
                <div class="p-8">
                    ...
                    <select name="course_ids[]" multiple size="8" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-medium text-text-main text-sm">
                        @foreach($courses as $c)
                            <option value="{{ $c->id }}" class="py-1.5 px-2 rounded-lg hover:bg-gray-100">{{ $c->title }}</option>
                        @endforeach
                    </select>
