...
@section('content')
<form method="POST" action="{{ route('admin.plans.update',$plan) }}" class="space-y-5 bg-white p-6 rounded shadow max-w-3xl">
  @csrf @method('PUT')
  ...
  <div class="grid grid-cols-2 gap-4">
    ...
    <div>
      ...
      <input type="number" name="price" class="w-full border rounded px-3 py-2" value="{{ old('price',$plan->price) }}" min="0" required>
  ...
  <div>
    ...
    <select name="period" class="w-full border rounded px-3 py-2" required>
      <option value="monthly" @selected(old('period',$plan->period)==='monthly')>Monthly</option>
      <option value="yearly"  @selected(old('period',$plan->period)==='yearly')>Yearly</option>
    </select>
  ...
  <div id="features-wrap">
    ...
    <div class="space-y-2">
      @foreach((array)($plan->features ?? []) as $f)
        <div><input name="features[]" class="w-full border rounded px-3 py-2" value="{{ $f }}"></div>
      @endforeach
      @if(empty($plan->features))
        <div><input name="features[]" class="w-full border rounded px-3 py-2" placeholder="Feature"></div>
      @endif
    </div>
  ...
  <div class="flex items-center gap-2">
    <a href="{{ route('admin.plans.index') }}" class="px-4 py-2 rounded border">Back</a>
