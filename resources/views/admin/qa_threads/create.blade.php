...
@section('title', 'Buat Q&A Thread Baru — Admin')
...
<div class="max-w-4xl mx-auto pb-12 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Buat Thread Diskusi</h1>
            <p class="text-sm text-text-soft mt-1">Mulai topik diskusi baru atas nama pengguna tertentu.</p>
        </div>
    ...
    @if ($errors->any())
        <div class="bg-red-50 border border-red-200 rounded-2xl p-4 flex gap-3 text-sm text-red-800 shadow-sm">
            ...
            <div>
                <p class="font-bold">Gagal menyimpan topik! Silakan periksa isian Anda:</p>
    ...
    <form method="POST" action="{{ route('admin.qa-threads.store') }}" class="space-y-8">
        @csrf
        ...
        <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-8 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Misal: Bagaimana cara mengonfigurasi database?"
                ...
                <div>
                    ...
                    <textarea name="body" rows="6" required placeholder="Tuliskan pertanyaan atau topik diskusi secara lengkap..."
                              class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-medium text-text-main leading-relaxed resize-y">{{ old('body') }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-bold text-text-main mb-2">Status Awal</label>
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4">
            <a href="{{ route('admin.qa-threads.index') }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors text-center">
                Batal
            </a>
