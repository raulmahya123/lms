...
@section('content')
    <div x-data="{ q: @js(request('q') ?? ''), showFilters: {{ request()->hasAny(['q', 'quiz_id']) ? 'true' : 'false' }} }" class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" @click="showFilters=!showFilters"
                    ...
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M3 5.25A.75.75 0 0 1 3.75 4.5h16.5a.75.75 0 0 1 .6 1.2L15 12.302v5.823a.75.75 0 0 1-1.08.67l-3-1.5A.75.75 0 0 1 10.5 17v-4.698L3.15 5.7a.75.75 0 0 1-.15-.45Z" />
                    </svg>
                    Filters
                </button>
            </div>
        ...
        {{-- FILTER FORM --}}
        <form id="q-filters" method="GET" x-show="showFilters" x-transition
            ...
            <div>
                ...
                <div class="relative">
                    <input type="text" name="q" x-model="q" value="{{ request('q') }}"
                        placeholder="Cari teks prompt…" class="w-full rounded-xl border pl-10 pr-3 py-2">
                    <svg class="h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 opacity-60" viewBox="0 0 24 24"
                        fill="currentColor">
                        <path fill-rule="evenodd"
                            d="M10 3.75a6.25 6.25 0 1 1 3.938 11.171l3.07 3.07a.75.75 0 1 1-1.06 1.06l-3.07-3.07A6.25 6.25 0 0 1 10 3.75Zm-4.75 6.25a4.75 4.75 0 1 0 9.5 0 4.75 4.75 0 0 0-9.5 0Z"
                            clip-rule="evenodd" />
                    </svg>
                </div>
            </div>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.questions.destroy', $item) }}" method="POST"
                                            ...
                                            data-title="{{ $item->title ?? ($item->question ?? 'ID: ' . $item->id) }}">
                                            @csrf @method('DELETE')

                                            <button type="button" {{-- penting: button, bukan submit --}}
                                                class="js-delete-btn inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-red-200 text-red-700 hover:bg-red-50"
                                                title="Delete">
                                                Delete
                                            </button>
                                        </form>
    ...
    @push('scripts')
        ...
        <script>
            (function() {
                    ...
                    document.querySelectorAll('.js-delete-btn').forEach(btn => {
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            Swal.fire({
                                ...
                                html: `Pertanyaan <b>${title}</b> akan dihapus permanen.`,
                                icon: 'warning',
                                ...
                                if (res.isConfirmed) {
                                    if (!form.dataset.submitting) { // anti double-submit
                ...
                // jika pakai Turbo/Livewire/htmx: re-bind saat navigasi dinamis
