...
@section('title', 'Edit Template Sertifikat — Admin')
...
<div class="max-w-4xl mx-auto space-y-8 pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Perbarui Template</h1>
            <p class="text-sm text-text-soft mt-1">Ubah konfigurasi desain dan pemetaan (mapping) data pada template ini.</p>
        </div>
        
        <div class="shrink-0 flex items-center gap-3">
            <a href="{{ route('admin.certificate-templates.show', $template) }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-colors">
    ...
    <form method="POST" action="{{ route('admin.certificate-templates.update', $template) }}" class="space-y-8">
        @csrf @method('PUT')
            ...
            <div class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="url" name="background_url" value="{{ old('background_url', $template->background_url) }}" placeholder="https://contoh.com/bg.png"
        ...
        <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-8 space-y-6">
                <div>
                    ...
                    <textarea name="fields_json" rows="5"
                              class="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 focus:ring-2 focus:ring-tosca outline-none transition-all font-mono text-sm text-gray-100 placeholder-gray-600 leading-relaxed custom-scrollbar">{{ old('fields_json', is_string($template->fields_json) ? $template->fields_json : json_encode($template->fields_json, JSON_PRETTY_PRINT)) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-bold text-text-main mb-2">Struktur Grafis Vektor (SVG JSON - Opsional)</label>
                    <textarea name="svg_json" rows="5"
                              class="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 focus:ring-2 focus:ring-tosca outline-none transition-all font-mono text-sm text-gray-100 placeholder-gray-600 leading-relaxed custom-scrollbar">{{ old('svg_json', is_string($template->svg_json) ? $template->svg_json : json_encode($template->svg_json, JSON_PRETTY_PRINT)) }}</textarea>
                </div>
            ...
            <div class="p-8">
                <div class="flex items-center gap-4">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" @checked(old('is_active', $template->is_active))>
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4">
            <a href="{{ route('admin.certificate-templates.show', $template) }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors text-center">
                Batal
            </a>
