...
<div x-data class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  <div class="mb-6 sm:mb-8">
    <div class="flex items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight text-coal-900">Edit Course</h1>
        <p class="mt-1 text-sm text-coal-500">Perbarui detail kursus. Anda bisa ganti cover atau pertahankan yang lama.</p>
      </div>
  ...
  <form method="POST" action="{{ route('admin.courses.update',$course) }}"
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
      ...
      <div class="sm:col-span-2">
        ...
        <textarea name="description" rows="4"
                  class="w-full rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-blue-600">{{ old('description',$course->description) }}</textarea>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      >
        <label class="mb-1.5 block text-sm font-medium">Ganti Cover (opsional)</label>
        ...
        <template x-if="preview">
          <div class="mt-3">
            ...
            <p class="mt-1 text-xs text-coal-500">Preview cover baru (belum disimpan).</p>
          </div>
        </template>
