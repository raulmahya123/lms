...
<div x-data class="max-w-4xl mx-auto space-y-8 pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Edit Kursus</h1>
            <p class="text-sm text-text-soft mt-1">Ubah informasi, cover, atau status publikasi kursus ini.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.courses.update', $course) }}" enctype="multipart/form-data" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-8 space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" 
                                  class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all placeholder-gray-400 resize-y leading-relaxed">{{ old('description', $course->description) }}</textarea>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div x-data="{
                        ...
                        <label class="block text-sm font-bold text-text-main mb-2">Ganti Cover (Opsional)</label>
                        ...
                        <div @dragover.prevent="isDropping = true"
                            ...
                            <div class="pointer-events-none flex flex-col items-center justify-center gap-3">
                                ...
                                <div>
                                    <p class="font-bold text-text-main text-sm">Klik atau seret gambar baru</p>
                        ...
                        <template x-if="preview">
                            <div class="mt-4 p-4 border border-gray-200 rounded-2xl bg-white shadow-sm inline-block w-full">
                                <p class="text-xs font-bold text-tosca uppercase tracking-wider mb-2">Pratinjau Cover Baru</p>
                                <img :src="preview" alt="Preview baru" class="h-40 w-full object-cover rounded-xl border border-gray-100">
                            </div>
                        </template>
                    ...
                    @if($course->cover)
                        <div>
                            <label class="block text-sm font-bold text-text-main mb-2">Cover Saat Ini</label>
                            <div class="p-4 border border-gray-200 rounded-2xl bg-gray-50">
                                <img src="{{ asset('storage/' . ltrim($course->cover, '/')) }}" alt="Current Cover" class="h-48 w-full object-cover rounded-xl border border-gray-200">
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Harga --}}
                    <div x-data="{ isFree: {{ old('is_free', $course->is_free ?? 1) ? 'true' : 'false' }} }" class="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" class="sr-only peer" @change="document.getElementById('publishField').value = $event.target.checked ? 1 : 0" {{ old('is_published', $course->is_published) ? 'checked' : '' }}>
