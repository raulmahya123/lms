            ...
            <div>
                <h1 class="text-2xl font-extrabold tracking-wide flex items-center gap-2">
                    <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M3.75 5.25A2.25 2.25 0 0 1 6 3h4.5A2.25 2.25 0 0 1 12.75 5.25v13.5A2.25 2.25 0 0 0 10.5 16.5H6A2.25 2.25 0 0 0 3.75 18.75V5.25Zm9 0A2.25 2.25 0 0 1 15 3h4.5A2.25 2.25 0 0 1 21.75 5.25v13.5A2.25 2.25 0 0 0 19.5 16.5H15a2.25 2.25 0 0 0-2.25 2.25V5.25Z"/>
                    </svg>
                    Courses
                </h1>
                    ...
                    <tbody class="[&>tr:hover]:bg-gray-50">
                        @forelse($courses as $c)
                            <tr class="border-t align-top">
                                {{-- Cover: single field `cover` (relative path under storage) + show filename --}}
                                <td class="p-3 align-top">
                                    @php
                                        // Normalisasi ke "covers/xxx.png"
                                        $rel = ltrim($c->cover ?? '', '/');
                                        // Jika user pernah menyimpan "storage/..." → buang prefix
                                    ...
                                    @if ($src && $exists)
                                        <img src="{{ $src }}" alt="Cover {{ $c->title }}"
                                             class="h-12 w-20 object-cover rounded border">
                                        <div class="mt-1 text-[10px] text-gray-500 truncate max-w-[7rem]" title="{{ $filename }}">
                                            {{ $filename }}
                                        </div>
                                    @else
                                        ...
                                        @if($filename)
                                            <div class="mt-1 text-[10px] text-red-600 truncate max-w-[7rem]" title="Missing: {{ $filename }}">
                                                Missing: {{ $filename }}
                                            </div>
                                        @endif
                                    @endif
                                ...
                                <td class="p-3">
                                    @if ($c->is_free)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full bg-emerald-100 text-emerald-800">
                                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M3.75 7.5A2.25 2.25 0 0 1 6 5.25h12A2.25 2.25 0 0 1 20.25 7.5v2.25a1.5 1.5 0 0 0 0 3V15A2.25 2.25 0 0 1 18 17.25H6A2.25 2.25 0 0 1 3.75 15v-2.25a1.5 1.5 0 0 0 0-3V7.5Z"/>
                                            </svg>
                                            Gratis
                                        </span>
                                    @else
                                        <div class="inline-flex items-center gap-2 rounded-xl border px-2 py-1 bg-white">
                                            <svg class="w-4 h-4 opacity-60" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M12 3.75a8.25 8.25 0 1 0 8.25 8.25A8.26 8.26 0 0 0 12 3.75Zm.75 4.5a.75.75 0 0 0-1.5 0v.29a3 3 0 0 0-2.25 2.91.75.75 0 0 0 1.5 0 1.5 1.5 0 1 1 1.5 1.5 3 3 0 1 0 3 3 .75.75 0 0 0-1.5 0 1.5 1.5 0 1 1-1.5-1.5 3 3 0 0 0 0-6Z"/>
                                            </svg>
                                ...
                                <td class="p-3">
                                    <div class="inline-flex items-center gap-2 rounded-xl border px-2 py-1 bg-white">
                                        <svg class="w-4 h-4 opacity-60" viewBox="0 0 24 24" fill="currentColor">
                                            <path d="M12 2.75 2.75 7.5 12 12.25 21.25 7.5 12 2.75Zm0 9.5L2.75 17l9.25 4.75L21.25 17 12 12.25Z"/>
                                        </svg>
                                ...
                                <td class="p-3">
                                    @if ($c->is_published)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-800">
                                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M12 2.25a9.75 9.75 0 1 1 0 19.5 9.75 9.75 0 0 1 0-19.5Zm-1.03 12.03 4.47-4.47a.75.75 0 0 0-1.06-1.06l-3.94 3.94-1.41-1.41a.75.75 0 0 0-1.06 1.06l1.94 1.94a.75.75 0 0 0 1.06 0Z"/>
                                            </svg>
                                            Published
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">
                                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M4.5 6.75A2.25 2.25 0 0 1 6.75 4.5h10.5A2.25 2.25 0 0 1 19.5 6.75V15a.75.75 0 0 1-.22.53l-4.5 4.5a.75.75 0 0 1-1.28-.53V15.75H6.75A2.25 2.25 0 0 1 4.5 13.5v-6.75Z"/>
                                            </svg>
                                            Draft
                                        </span>
                                    @endif
                                ...
                                <td class="p-3">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.courses.destroy', $c) }}"
                                            ...
                                            <button type="button" {{-- button, bukan submit --}}
                                                    ...
                                                    title="Delete">
                                                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                                    <path d="M9.75 3a1 1 0 0 0-.94.66L8.5 4.5H6a.75.75 0 0 0 0 1.5h12a.75.75 0 0 0 0-1.5h-2.5l-.31-.84a1 1 0 0 0-.94-.66h-4.5Z"/>
                                                </svg>
                                                Delete
                                            </button>
                                        </form>
                                    ...
                                    <div class="flex flex-col items-center justify-center text-center gap-3">
                                        <div class="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center">
                                            <svg class="w-8 h-8 opacity-50" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M6.75 3A2.75 2.75 0 0 0 4 5.75v12.5A2.75 2.75 0 0 0 6.75 21h10.5A2.75 2.75 0 0 0 20 18.25V9.5a.75.75 0 0 0-.22-.53l-5.75-5.75A.75.75 0 0 0 13.5 3h-6.75Z"/>
                                            </svg>
                                        </div>
                                        <div class="text-lg font-semibold">Belum ada course</div>
                                        <p class="text-sm opacity-70 max-w-md">Tambahkan course pertama agar tim bisa mulai membuat modul & materi.</p>
                                        <a href="{{ route('admin.courses.create') }}"
