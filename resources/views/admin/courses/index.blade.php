...
<div class="space-y-6">
  ...
  <form method="GET" action="{{ route('admin.courses.index') }}"
        class="rounded-2xl border bg-white p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
    <div>
    ...
    <div>
      ...
      <div class="relative">
        <select name="published" class="w-full border rounded-xl pl-10 pr-3 py-2">
  ...
  <div class="rounded-2xl border bg-white overflow-x-auto">
    ...
    <table class="w-full min-w-[820px] text-sm">
      <thead class="bg-gray-100 text-gray-700 sticky top-0">
        <tr>
          ...
          <th class="p-3 text-center w-44">Action</th>
        </tr>
      </thead>
      <tbody class="[&>tr:hover]:bg-gray-50">
        @forelse($courses as $c)
          <tr class="border-t align-top">
            ...
            <td class="p-3">
              <div class="inline-flex items-center gap-1 rounded-xl border px-2 py-1 bg-white">
                {{-- Stack Icon --}}
                <svg class="w-4 h-4 opacity-70" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2.75 2.75 7.5 12 12.25 21.25 7.5 12 2.75Zm0 9.5L2.75 17l9.25 4.75L21.25 17 12 12.25Zm-9.25 6L12 22.5l9.25-4.25"/>
                </svg>
    ...
    <div class="px-4 py-3 border-t bg-gray-50 flex flex-col md:flex-row items-center justify-between gap-3">
      <div class="text-sm opacity-70">
        Showing
        <span class="font-semibold">{{ $courses->firstItem() ?? 0 }}</span>
        to
        <span class="font-semibold">{{ $courses->lastItem() ?? 0 }}</span>
        of
        <span class="font-semibold">{{ $courses->total() }}</span>
        results
      </div>
