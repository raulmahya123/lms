...
<div class="max-w-3xl mx-auto space-y-8 pb-12">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Edit Resource</h1>
      <p class="text-sm text-text-soft mt-1">Ubah judul, link referensi, atau tipe materi untuk pelajaran ini.</p>
    </div>
    <div class="shrink-0 flex gap-3">
      <a href="{{ route('admin.resources.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-colors">
  ...
  <form method="POST" action="{{ route('admin.resources.update', $resource) }}" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
    @csrf
    @method('PUT')

    <div class="p-8 space-y-8">
      
      {{-- Penempatan Pelajaran --}}
      <div>
        <label class="block text-sm font-bold text-text-main mb-2">Pelajaran (Lesson) <span class="text-red-500">*</span></label>
        <div class="relative">
          <select name="lesson_id" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-bold text-text-main appearance-none cursor-pointer" required>
            ...
            @foreach($lessons as $lesson)
              <option value="{{ $lesson->id }}" @selected(old('lesson_id', $resource->lesson_id) == $lesson->id)>{{ $lesson->title }}</option>
            @endforeach
          </select>
      ...
      {{-- Judul Resource --}}
      <div>
        <label class="block text-sm font-bold text-text-main mb-2">Judul Resource <span class="text-red-500">*</span></label>
        <input type="text" name="title" value="{{ old('title', $resource->title) }}" 
               ...
               placeholder="Contoh: Modul Dasar PDF" required>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        {{-- URL --}}
        <div>
          <label class="block text-sm font-bold text-text-main mb-2">URL (Link) <span class="text-red-500">*</span></label>
          <div class="relative">
            ...
            <input type="url" name="url" value="{{ old('url', $resource->url) }}" placeholder="https://..." 
                   class="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 focus:bg-white focus:border-tosca focus:ring-2 focus:ring-tosca/20 outline-none transition-all font-medium" required>
          </div>
          @error('url') <p class="text-sm font-medium text-red-600 mt-1">{{ $message }}</p> @enderror
        </div>

        {{-- Type --}}
        <div>
          <label class="block text-sm font-bold text-text-main mb-2">Tipe Format</label>
          <input type="text" name="type" value="{{ old('type', $resource->type) }}" 
                 class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-bold text-text-main placeholder-gray-400" 
                 placeholder="Misal: pdf, video, link">
    ...
    <div class="p-6 border-t border-gray-50 bg-gray-50/30 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
      <a href="{{ route('admin.resources.index') }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors text-center">
        Batal
      </a>
