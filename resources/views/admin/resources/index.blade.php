...
@section('title', 'Manajemen Resource — Admin')
    ...
    <div x-data="{ q: @js(request('q') ?? ''), showFilters: false }" class="space-y-6 pb-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Manajemen Resource</h1>
                <p class="text-sm text-text-soft mt-1">Kelola lampiran file, referensi link, dan dokumen untuk melengkapi pelajaran.</p>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden flex flex-col">
            <div class="p-6 border-b border-gray-50 bg-softbg/30 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="text-sm font-bold text-text-main">Total {{ $resources->total() }} Resource</span>
                ...
                <table class="w-full text-left border-collapse min-w-[1000px]">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase tracking-wider text-text-soft">
                            <th class="px-6 py-4 font-bold">Informasi Resource</th>
                            <th class="px-6 py-4 font-bold">Pelajaran (Lesson)</th>
                            <th class="px-6 py-4 font-bold">Tipe & URL</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($resources as $res)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                {{-- Informasi Resource --}}
                                <td class="px-6 py-4">
                                    <div class="font-bold text-text-main text-base mb-1">{{ $res->title }}</div>
                                </td>

                                {{-- Pelajaran --}}
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-text-main truncate max-w-[250px]" title="{{ $res->lesson->title ?? '-' }}">
                                        {{ $res->lesson->title ?? '-' }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        @if(strtolower($res->type) === 'pdf' || strtolower($res->type) === 'document')
                                            <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border bg-red-50 text-red-600 border-red-100 w-16">
                                                {{ $res->type ?? '-' }}
                                            </span>
                                        @elseif(strtolower($res->type) === 'link' || strtolower($res->type) === 'url')
                                            <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border bg-blue-50 text-blue-600 border-blue-100 w-16">
                                                {{ $res->type ?? '-' }}
                                            </span>
                                        @else
                                            <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border bg-gray-100 text-gray-600 border-gray-200 w-16">
                                                {{ $res->type ?? '-' }}
                                            </span>
                                        @endif
                                        
                                        <a href="{{ $res->url }}" target="_blank" class="inline-flex items-center gap-1 text-sm font-bold text-tosca hover:text-tosca-dark transition-colors underline-offset-2 hover:underline truncate max-w-[200px]" title="{{ $res->url }}">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.resources.show', $res) }}" class="inline-flex items-center justify-center p-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-300 hover:text-blue-600 transition-colors" title="Lihat Resource">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </a>
                                        <a href="{{ route('admin.resources.edit', $res) }}" class="inline-flex items-center justify-center p-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-300 hover:text-tosca transition-colors" title="Edit Resource">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form method="POST" action="{{ route('admin.resources.destroy', $res) }}" class="inline js-delete-form" data-title="{{ $res->title }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="js-delete-btn inline-flex items-center justify-center p-2 bg-white border border-gray-200 text-gray-400 rounded-lg hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-colors" title="Hapus Resource">
                                ...
                                <td colspan="4">
                                    <div class="py-16 text-center flex flex-col items-center justify-center">
                                        <div class="w-20 h-20 rounded-3xl bg-softbg flex items-center justify-center mb-6">
                                            <svg class="w-10 h-10 text-tosca" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                                        </div>
                                        <h3 class="text-xl font-bold text-text-main mb-2">Belum Ada Resource</h3>
                                        <p class="text-text-soft max-w-sm mb-6">Tambahkan materi referensi, file PDF, atau dokumen pendukung untuk pelajaran yang ada.</p>
                                        <a href="{{ route('admin.resources.create') }}" class="px-6 py-3 rounded-xl bg-tosca text-white font-bold hover:bg-tosca-dark transition-colors shadow-sm shadow-tosca/20 flex items-center gap-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                    ...
                    document.querySelectorAll('.js-delete-btn').forEach(btn => {
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            Swal.fire({
                                ...
                                html: `Resource <br><b class="text-text-main">${title}</b><br> akan dihapus permanen dari sistem.`,
                                icon: 'warning',
