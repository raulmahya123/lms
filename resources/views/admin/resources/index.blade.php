            ...
            <div>
                <h1 class="text-2xl font-extrabold tracking-wide flex items-center gap-2">
                    ...
                    <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M6 4.5A2.25 2.25 0 0 0 3.75 6.75v11.5A2.25 2.25 0 0 0 6 20.5h12a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 18 4.5H6Zm0 1.5h12c.414 0 .75.336.75.75v10.25a.75.75 0 0 1-.75.75H6a.75.75 0 0 1-.75-.75V6.75c0-.414.336-.75.75-.75Z" />
                    </svg>
                    Resources
                </h1>
                <p class="text-sm opacity-70">Kelola resource tambahan per lesson. Bisa berupa file, link, atau dokumen
                    referensi.</p>
            </div>
                            ...
                            <tr class="border-t">
                                ...
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.resources.destroy', $res) }}" method="POST"
                                            class="inline js-delete-form"
                                            data-title="{{ $res->title ?? ($res->name ?? 'ID: ' . $res->id) }}">
                                            @csrf @method('DELETE')
                ...
                function bindDeleteButtons() {
                    document.querySelectorAll('.js-delete-btn').forEach(btn => {
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            const title = form?.dataset.title || 'resource ini';

                            Swal.fire({
                                title: 'Hapus resource?',
                                html: `Resource <b>${title}</b> akan dihapus permanen.`,
                                icon: 'warning',
                                ...
                                if (res.isConfirmed) {
                                    if (!form.dataset.submitting) { // anti double submit + UX kecil
