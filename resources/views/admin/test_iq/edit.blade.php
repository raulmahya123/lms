...
@section('title','Edit Tes IQ — Admin')
...
@php
  ...
  if (!is_array($initialQuestions)) {
      $fromModel = $testIq->questions ?? null;
  ...
  $oldNorm = old('norm_table_json', null);
  if ($oldNorm !== null) {
      $initialNormJson = $oldNorm;
  } else {
      $norm = data_get($testIq, 'meta.norm_table');
...
<div class="max-w-5xl mx-auto space-y-8 pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Edit Tes IQ</h1>
            <p class="text-sm text-text-soft mt-1">Ubah pengaturan dasar, waktu, norma, atau perbarui bank soal yang tersedia.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.test-iq.update', $testIq) }}" class="space-y-8" id="iqTestForm">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
            <div class="px-8 py-5 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $testIq->is_active) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" value="{{ old('title', $testIq->title) }}" 
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3" 
                                  class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-medium text-text-main placeholder-gray-400 resize-y">{{ old('description', $testIq->description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-text-main mb-2">Durasi Pengerjaan</label>
                        <div class="relative">
                            <input type="number" name="duration_minutes" min="0" value="{{ old('duration_minutes', $testIq->duration_minutes) }}" 
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="number" name="cooldown_value" min="0" value="{{ $cooldownValue }}" 
                            ...
                            <select name="cooldown_unit" class="w-1/2 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-medium text-text-main appearance-none cursor-pointer">
                                <option value="day"   {{ $cooldownUnit==='day'?'selected':'' }}>Hari</option>
                                <option value="week"  {{ $cooldownUnit==='week'?'selected':'' }}>Minggu</option>
                                <option value="month" {{ $cooldownUnit==='month'?'selected':'' }}>Bulan</option>
                            </select>
                        </div>
                    </div>
                </div>
        ...
        {{-- Meta Cooldown (Optional UI Helpers) --}}
        <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
            <div class="px-8 py-5 border-b border-gray-50 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-base font-extrabold text-text-main flex items-center gap-2">
                        ...
                        Pengaturan Waktu Tunggu Lanjutan (Meta)
                    </h2>
                    <p class="text-sm text-text-soft mt-1">Konfigurasi mendetail untuk timer spesifik pada user interface (opsional).</p>
                </div>
            </div>

            <div class="p-8 grid grid-cols-1 md:grid-cols-5 gap-6">
                <div>
                    <label class="block text-xs font-bold text-text-soft uppercase tracking-wider mb-2">Detik Langsung</label>
                    <input type="number" name="cooldown_seconds" min="0" value="{{ is_null($metaCooldownSeconds) ? '' : $metaCooldownSeconds }}" placeholder="Contoh: 3600"
                           class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-text-main text-center">
                    <p class="text-[10px] text-gray-400 mt-1.5 leading-tight">Biarkan kosong agar sistem otomatis menghitung dari kombinasi input di samping.</p>
                </div>

                <div>
                    <label class="block text-xs font-bold text-text-soft uppercase tracking-wider mb-2">Hari</label>
                    <input type="number" name="cooldown_days" min="0" value="{{ $metaCooldownRaw['days'] }}" 
                           class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-text-main text-center">
                </div>

                <div>
                    <label class="block text-xs font-bold text-text-soft uppercase tracking-wider mb-2">Jam</label>
                    <input type="number" name="cooldown_hours" min="0" value="{{ $metaCooldownRaw['hours'] }}" 
                           class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-text-main text-center">
                </div>

                <div>
                    <label class="block text-xs font-bold text-text-soft uppercase tracking-wider mb-2">Menit</label>
                    <input type="number" name="cooldown_minutes" min="0" value="{{ $metaCooldownRaw['minutes'] }}" 
                           class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-text-main text-center">
                </div>

                <div>
                    <label class="block text-xs font-bold text-text-soft uppercase tracking-wider mb-2">Hapus Konfigurasi</label>
                    <div class="relative">
                        <select name="cooldown_clear" class="w-full bg-red-50 text-red-700 border border-red-200 rounded-xl px-4 py-3 focus:border-red-500 focus:ring-4 focus:ring-red-500/10 outline-none transition-all font-bold appearance-none cursor-pointer">
                            <option value="">Tidak</option>
                            <option value="1" {{ old('cooldown_clear') ? 'selected' : '' }}>Ya, Hapus Data Meta</option>
                        </select>
        ...
        <div x-data="questionBuilder({ initial: @js($initialQuestions) })" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                <template x-for="(q, qi) in questions" :key="q.id">
                    <div class="p-8 hover:bg-gray-50/30 transition-colors">
