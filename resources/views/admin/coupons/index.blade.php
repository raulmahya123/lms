    ...
    <div>
      <h1 class="text-2xl font-extrabold tracking-wide flex items-center gap-2">
        ...
        <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
          <path d="M3 7a2 2 0 0 1 2-2h3a2 2 0 1 0 4 0h7a2 2 0 0 1 2 2v2a2 2 0 1 1 0 4v2a2 2 0 0 1-2 2h-7a2 2 0 1 0-4 0H5a2 2 0 0 1-2-2V7Z"/>
        </svg>
        Coupons
      </h1>
      <p class="text-sm opacity-70">Kelola kupon: cari kode, filter status aktif/kadaluarsa/dijadwalkan, lihat pemakaian, edit & hapus.</p>
    </div>
          ...
          @forelse($coupons as $c)
            @php
              ...
              $statusBadge = $isActive
                ...
                : ($isExpired ? ['bg-red-100','text-red-800','Expired']
            ...
            <tr class="border-t">
              ...
              <td class="p-3">
                <div class="flex items-center gap-2">
                  ...
                  @if($c->max_discount_amount)
                    <span class="text-[10px] px-1.5 py-0.5 rounded border text-gray-600">Cap: {{ number_format($c->max_discount_amount,0,',','.') }}</span>
                  @endif
                </div>
              </td>
              <td class="p-3">
                @if(!is_null($c->discount_percent))
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-xs">
              ...
              <td class="p-3">
                <div class="flex items-center gap-2">
                  <span class="px-2 py-0.5 rounded-full text-xs {{ $statusBadge[0] }} {{ $statusBadge[1] }}">{{ $statusBadge[2] }}</span>
              ...
              <td class="p-3 text-center">
                <div class="flex items-center justify-center gap-2">
                  ...
                  <form method="POST" action="{{ route('admin.coupons.destroy',$c) }}"
                        onsubmit="return confirm('Delete coupon?')" class="inline">
                    @csrf @method('DELETE')
