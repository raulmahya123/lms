            ...
            <div>
                ...
                <div class="relative">
                    <input type="text" name="q" x-model="q" value="{{ request('q') }}"
                        placeholder="Ketik nama atau email…" class="w-full border rounded-xl pl-10 pr-3 py-2">
                    <svg class="w-5 h-5 absolute left-3 top-2.5 opacity-60" viewBox="0 0 24 24" fill="currentColor">
                        <path
                    ...
                    <tbody class="[&>tr:hover]:bg-gray-50">
                        @forelse($items as $m)
                            ...
                            <tr class="border-t">
                                ...
                                <td class="p-3">
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $badge[0] }} {{ $badge[1] }}">
                                        {{ $badge[2] }}
                                    </span>
                                </td>
                                ...
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.memberships.destroy', $m) }}"
                                            ...
                                            data-title="Membership {{ $m->plan->name ?? '—' }} {{ optional($m->activated_at)->format('Y-m-d') ? '(' . $m->activated_at->format('Y-m-d') . ')' : '' }}">
                                            @csrf @method('DELETE')
