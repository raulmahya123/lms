...
@section('title', 'Manajemen Modul — Admin')
    ...
    <div x-data="{ q: @js(request('q') ?? ''), showFilters: false }" class="space-y-6 pb-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Manajemen Modul</h1>
                <p class="text-sm text-text-soft mt-1">Kelola modul dan urutan pembelajaran untuk setiap kursus.</p>
            </div>
        ...
        <form method="GET" x-show="showFilters" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" style="display: none;"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="relative">
                        <select name="course_id" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-medium text-text-main appearance-none cursor-pointer">
                            ...
                            @foreach ($__courses as $c)
                                <option value="{{ $c->id }}" @selected(request('course_id') == $c->id)>{{ Str::limit($c->title, 40) }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    <label class="block text-xs font-bold text-text-soft uppercase tracking-wider mb-2">Cari Judul Modul</label>
                    <div class="relative">
        ...
        <div class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden flex flex-col">
            <div class="p-6 border-b border-gray-50 bg-softbg/30 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="text-sm font-bold text-text-main">Total {{ $modules->total() }} Modul</span>
                ...
                <div class="text-xs font-bold text-text-soft uppercase tracking-wider">
                    Halaman {{ $modules->currentPage() }} dari {{ $modules->lastPage() }}
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase tracking-wider text-text-soft">
                            <th class="px-6 py-4 font-bold">Judul Modul</th>
                            <th class="px-6 py-4 font-bold">Terkait Kursus</th>
                            <th class="px-6 py-4 font-bold text-center">Urutan (Order)</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($modules as $m)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-text-main text-base mb-1">{{ $m->title }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @if ($m->course)
                                        <div class="flex items-center gap-2 text-sm font-bold text-text-main">
                                            ...
                                            <span class="truncate max-w-[250px]">{{ $m->course->title }}</span>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.modules.edit', $m) }}" class="inline-flex items-center justify-center p-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-300 hover:text-tosca transition-colors" title="Edit Modul">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form method="POST" action="{{ route('admin.modules.destroy', $m) }}" class="inline js-delete-form" data-title="{{ $m->title }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="js-delete-btn inline-flex items-center justify-center p-2 bg-white border border-gray-200 text-gray-400 rounded-lg hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-colors" title="Hapus Modul">
                                ...
                                <td colspan="4">
                                    <div class="py-16 text-center flex flex-col items-center justify-center">
                                        <div class="w-20 h-20 rounded-3xl bg-softbg flex items-center justify-center mb-6">
                                            <svg class="w-10 h-10 text-tosca" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg>
                                        </div>
                                        <h3 class="text-xl font-bold text-text-main mb-2">Belum Ada Modul</h3>
                                        <p class="text-text-soft max-w-sm mb-6">Mulai tambahkan modul untuk menyusun struktur pembelajaran yang terstruktur di dalam kursus.</p>
                                        <a href="{{ route('admin.modules.create') }}" class="px-6 py-3 rounded-xl bg-tosca text-white font-bold hover:bg-tosca-dark transition-colors shadow-sm shadow-tosca/20 flex items-center gap-2">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                            Buat Modul Pertama
                                        </a>
                                    </div>
                ...
                function bindDeleteButtons() {
                    document.querySelectorAll('.js-delete-btn').forEach(btn => {
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            Swal.fire({
                                title: 'Hapus Modul?',
                                html: `Semua materi/pelajaran di dalam modul <br><b class="text-text-main">${title}</b><br> akan dihapus permanen.`,
                                icon: 'warning',
