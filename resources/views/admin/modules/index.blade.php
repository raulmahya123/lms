    ...
    <div x-data="{ q: @js(request('q') ?? ''), showFilters: false }" class="space-y-6">
            ...
            <div>
                <h1 class="text-2xl font-extrabold tracking-wide flex items-center gap-2">
                    ...
                    <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M4 6.75A2.75 2.75 0 0 1 6.75 4h10.5A2.75 2.75 0 0 1 20 6.75v10.5A2.75 2.75 0 0 1 17.25 20H6.75A2.75 2.75 0 0 1 4 17.25V6.75Zm3.25.75a.75.75 0 0 0 0 1.5h9.5a.75.75 0 0 0 0-1.5h-9.5Zm0 4a.75.75 0 0 0 0 1.5h9.5a.75.75 0 0 0 0-1.5h-9.5Zm0 4a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5Z" />
                    </svg>
                    Modules
                </h1>
        ...
        <form method="GET" x-show="showFilters" x-transition
            ...
            <div class="col-span-1">
                ...
                <div class="relative">
                    <input type="text" name="q" x-model="q" placeholder="Cari judul module…"
                    ...
                    <tbody class="[&>tr:hover]:bg-gray-50">
                        @forelse($modules as $m)
                            <tr class="border-t">
                                    ...
                                    @if ($m->course)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="truncate max-w-[320px]"
                                                title="{{ $m->course->title }}">{{ $m->course->title }}</span>
                                        </div>
                                    @else
                                ...
                                <td class="p-3">
                                    <div class="truncate max-w-[420px] font-medium" title="{{ $m->title }}">
                                        {{ $m->title }}</div>
                                </td>
                                <td class="p-3">
                                    <div class="inline-flex items-center gap-2 rounded-xl border px-2 py-1 bg-white">
                                        ...
                                        <svg class="w-4 h-4 opacity-60" viewBox="0 0 24 24" fill="currentColor">
                                            <path
                                                d="M6 12.75a.75.75 0 0 1 .75-.75h10.5a.75.75 0 0 1 0 1.5H6.75a.75.75 0 0 1-.75-.75ZM6 7.5a.75.75 0 0 1 .75-.75h10.5a.75.75 0 0 1 0 1.5H6.75A.75.75 0 0 1 6 7.5Zm0 10.5a.75.75 0 0 1 .75-.75h7.5a.75.75 0 0 1 0 1.5h-7.5a.75.75 0 0 1-.75-.75Z" />
                                        </svg>
                                ...
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.modules.destroy', $m) }}"
                                            class="js-delete-form inline" data-title="{{ $m->title }}">
                                            @csrf @method('DELETE')
                                    ...
                                    <div class="flex flex-col items-center justify-center text-center gap-3">
                                        <div class="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center">
                                            ...
                                            <svg class="w-8 h-8 opacity-50" viewBox="0 0 24 24" fill="currentColor">
                                                <path
                                                    d="M4 6.75A2.75 2.75 0 0 1 6.75 4h10.5A2.75 2.75 0 0 1 20 6.75v10.5A2.75 2.75 0 0 1 17.25 20H6.75A2.75 2.75 0 0 1 4 17.25V6.75Zm3.25 1a.75.75 0 0 0 0 1.5h9.5a.75.75 0 0 0 0-1.5h-9.5Z" />
                                            </svg>
                                        </div>
                                        <div class="text-lg font-semibold">Belum ada module</div>
                                        <p class="text-sm opacity-70 max-w-md">Tambahkan modul pertama untuk course kamu.
                                            Mulai dari judul dan urutan tampil.</p>
                                        <a href="{{ route('admin.modules.create') }}"
    ...
    @push('scripts')
        ...
        <script>
            (function() {
                function bindDeleteButtons() {
                    document.querySelectorAll('.js-delete-btn').forEach(btn => {
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            Swal.fire({
                                title: 'Hapus module?',
                                html: `Module <b>${title}</b> akan dihapus permanen.`,
                                icon: 'warning',
                                ...
                                if (res.isConfirmed) {
                                    // optional: cegah double submit + ubah label tombol
                ...
                // kalau pakai Turbo/Livewire/htmx/Alpine route swap, rebind juga:
