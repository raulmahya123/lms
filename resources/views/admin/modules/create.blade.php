...
@section('title', 'Buat Modul Baru — Admin')
...
<div class="max-w-4xl mx-auto space-y-8 pb-12">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Tambah Modul</h1>
      <p class="text-sm text-text-soft mt-1">Tambahkan modul pembelajaran baru ke dalam struktur kursus.</p>
    </div>
  ...
  <form method="POST" action="{{ route('admin.modules.store') }}" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
    @csrf
    ...
    <div class="p-8 space-y-8">
      ...
      <div>
        ...
        <div>
          ...
          <div class="relative">
            <select name="course_id" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all font-bold text-text-main appearance-none cursor-pointer" required>
              ...
              @foreach($courses as $c)
                <option value="{{ $c->id }}" @selected(old('course_id')==$c->id)>{{ Str::limit($c->title, 70) }}</option>
              @endforeach
            </select>
          ...
          <p class="text-xs text-text-soft font-medium mt-2">Modul akan ditambahkan ke dalam kurikulum kursus yang dipilih.</p>
      ...
      <div>
        ...
        <div class="space-y-6">
          ...
          <div class="max-w-xs">
            ...
            <input type="number" name="ordering" min="0" value="{{ old('ordering', 0) }}" 
