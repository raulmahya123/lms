            ...
            <div>
                <h1 class="text-2xl font-extrabold tracking-wide flex items-center gap-2">
                    ...
                    <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M9 3.75A2.25 2.25 0 0 1 11.25 1.5h1.5A2.25 2.25 0 0 1 15 3.75h1.5A2.25 2.25 0 0 1 18.75 6v12A2.25 2.25 0 0 1 16.5 20.25h-9A2.25 2.25 0 0 1 5.25 18V6A2.25 2.25 0 0 1 7.5 3.75H9Zm1.5 0A.75.75 0 0 0 9.75 4.5h4.5a.75.75 0 0 0-.75-.75h-3Z" />
                    </svg>
                    Quizzes
                </h1>
                    ...
                    <tbody class="[&>tr:hover]:bg-gray-50">
                        @forelse($quizzes as $qz)
                            <tr class="border-t">
                                ...
                                <td class="p-3">
                                    <div class="truncate max-w-[520px] font-medium" title="{{ $qz->title }}">
                                        {{ $qz->title }}</div>
                                </td>
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.quizzes.destroy', $qz) }}"
                                            class="js-delete-form inline" data-title="{{ $qz->title }}">
                                            @csrf @method('DELETE')
                            ...
                            <tr>
                                <td colspan="4" class="p-10">
                                    <div class="flex flex-col items-center justify-center text-center gap-3">
                                        ...
                                        <p class="text-sm opacity-70 max-w-md">Tambahkan kuis untuk evaluasi pembelajaran
                                            di setiap lesson.</p>
                                        <a href="{{ route('admin.quizzes.create') }}"
            ...
            (function() {
                function bindDeleteButtons() {
                    document.querySelectorAll('.js-delete-btn').forEach(btn => {
                        if (btn.dataset.bound) return; // cegah double binding saat re-render
                        btn.dataset.bound = '1';
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            Swal.fire({
                                title: 'Hapus quiz?',
                                html: `Quiz <b>${title}</b> akan dihapus permanen.`,
                                icon: 'warning',
                ...
                // kalau pakai Turbo/Livewire/htmx/Alpine nav, re-bind juga:
