...
@section('title', 'Buat Pelajaran Baru — Admin')
...
<div class="max-w-4xl mx-auto space-y-8 pb-12">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-text-main tracking-tight">Tambah Pelajaran</h1>
      <p class="text-sm text-text-soft mt-1">Lengkapi materi pelajaran, video, dan informasi akses di bawah ini.</p>
    </div>
  ...
  <form method="POST" action="{{ route('admin.lessons.store') }}" class="bg-white rounded-3xl border border-gray-50 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
    @csrf
        ...
        <div class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="number" name="ordering" min="1" value="{{ old('ordering', 1) }}" 
          ...
          <div>
            ...
            <input type="text" name="title" value="{{ old('title') }}" 
                   ...
                   placeholder="Contoh: Pengenalan Konsep Dasar" required>
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="md:col-span-2">
            ...
            <textarea name="about" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all placeholder-gray-400 resize-y leading-relaxed" placeholder="Tuliskan ringkasan singkat materi...">{{ old('about') }}</textarea>
          ...
          <div>
            ...
            <textarea name="syllabus" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all placeholder-gray-400 resize-y leading-relaxed" placeholder="Garis besar topik pembelajaran...">{{ old('syllabus') }}</textarea>
          ...
          <div>
            ...
            <textarea name="reviews" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all placeholder-gray-400 resize-y leading-relaxed" placeholder="Catatan review atau testimoni terkait pelajaran ini...">{{ old('reviews') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          {{-- Tools Input --}}
          <div x-data="{
                items: (()=>{ const raw = @js(old('tools')); if (!raw) return []; try { const j=JSON.parse(raw); return Array.isArray(j)?j:[]; } catch(e) {} return String(raw).split(',').map(s=>s.trim()).filter(Boolean); })(),
                input:'',
            ...
            <div class="flex gap-2 mb-3">
              <input x-model="input" @keydown.enter.prevent="add()" type="text" class="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all" placeholder="Misal: Laptop, VS Code...">
          ...
          {{-- Benefits Input --}}
          <div x-data="{
                items: (()=>{ const raw = @js(old('benefits')); if (!raw) return []; try { const j=JSON.parse(raw); return Array.isArray(j)?j:[]; } catch(e) {} return String(raw).split(',').map(s=>s.trim()).filter(Boolean); })(),
                input:'',
                add(){ const v=this.input.trim(); if(!v) return; if(!this.items.includes(v)) this.items.push(v); this.input=''; },
                remove(i){ this.items.splice(i,1); }
              }">
            ...
            <div class="flex gap-2 mb-3">
              <input x-model="input" @keydown.enter.prevent="add()" type="text" class="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all" placeholder="Misal: Memahami dasar logika...">
              <button type="button" @click="add()" class="px-4 py-2 rounded-xl bg-gray-900 text-white font-bold hover:bg-black transition-colors shrink-0">Tambah</button>
            </div>
            <div class="flex flex-wrap gap-2">
      ...
      <div>
        ...
        <div class="space-y-6">
          <div>
            ...
            <textarea name="content" rows="6" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:bg-white focus:border-tosca focus:ring-4 focus:ring-tosca/10 outline-none transition-all placeholder-gray-400 resize-y font-mono text-sm leading-relaxed" placeholder="Tuliskan materi teks disini...">{{ old('content') }}</textarea>
            @error('content') <p class="text-sm font-medium text-red-600 mt-1">{{ $message }}</p> @enderror
          </div>

          <div x-data="{ urls: @js(old('content_url', [])) }" class="bg-gray-50 border border-gray-200 rounded-2xl p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div class="relative">
                ...
                <input type="url" name="drive_link" value="{{ old('drive_link') }}" placeholder="https://drive.google.com/..." 
            ...
            <div>
              ...
              <div class="relative">
                <select name="drive_status" class="w-full pl-4 pr-10 py-3 bg-white border border-gray-200 rounded-xl focus:bg-white focus:border-tosca focus:ring-2 focus:ring-tosca/20 outline-none transition-all font-bold text-text-main appearance-none cursor-pointer">
                  ...
                  <option value="pending" @selected(old('drive_status')==='pending')>Pending (Menunggu)</option>
                  <option value="approved" @selected(old('drive_status')==='approved')>Approved (Disetujui)</option>
                  <option value="rejected" @selected(old('drive_status')==='rejected')>Rejected (Ditolak)</option>
                </select>
                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                  <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </div>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6">
          <div class="flex items-start justify-between">
            ...
            <label class="relative inline-flex items-center cursor-pointer mt-1">
              <input type="checkbox" name="is_free" value="1" @checked(old('is_free')) class="sr-only peer">
