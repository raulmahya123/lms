...
<head>
  ...
  <style>
    ...
    .auth-input{
      height:52px;width:100%;border-radius:16px;border:1px solid rgba(15,157,138,.16);
      background:rgba(255,255,255,.88);padding:0 44px 0 44px;font-size:.925rem;
      color:#10201d;outline:none;transition:border-color .2s,box-shadow .2s,background .2s;
    }
    .auth-input:hover{border-color:rgba(15,157,138,.32)}
    .auth-input:focus{border-color:#0F9D8A;box-shadow:0 0 0 4px rgba(15,157,138,.12);background:#fff}
    ...
    .dark .auth-input:focus{background:rgba(255,255,255,.12);border-color:#0F9D8A}
...
<body
  ...
  class="min-h-screen bg-[#f7fbfa] text-ink antialiased dark:bg-[#071511] dark:text-white">

  <main class="min-h-screen lg:grid lg:grid-cols-[55%_45%]">
    <section class="@yield('visual_order', '') hidden min-h-screen overflow-hidden bg-[radial-gradient(circle_at_18%_18%,rgba(15,157,138,.24),transparent_28%),linear-gradient(135deg,#075E54,#10201d)] p-8 text-white lg:flex">
      <div class="relative flex w-full flex-col justify-between overflow-hidden rounded-[2rem] border border-white/12 bg-white/8 p-8 shadow-2xl backdrop-blur">
        ...
        <div class="absolute -bottom-24 left-16 h-72 w-72 rounded-full bg-[#0F9D8A]/20 blur-3xl"></div>
        ...
        <div class="relative z-10 flex items-center justify-between">
          <a href="{{ route('home') }}" class="flex items-center gap-3">
            ...
            <span class="text-xl font-black tracking-tight">BERKE<span class="text-[#9df4e7]">MAH</span></span>
          </a>
        ...
        <div class="relative z-10 my-10 max-w-xl">
          <p class="text-sm font-bold uppercase tracking-[.2em] text-[#9df4e7]">@yield('visual_kicker', 'Belajar online')</p>
