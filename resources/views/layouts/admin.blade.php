...
<body class="min-h-screen flex"
  ...
  $icon = function(string $label): string {
    $base = 'class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"';
    ...
    <header class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 shadow sticky top-0 z-30"
      ...
      <div class="flex items-center gap-2 sm:gap-3" x-data="{open:false}">
        ...
        <div class="relative">
          ...
          <div x-cloak x-show="open" @click.outside="open=false"
            ...
            <form method="POST" action="{{ route('logout') }}"> @csrf
              <button type="submit" class="w-full text-left px-4 py-2.5 hover:bg-red-500/10 text-red-600">
                Logout
              </button>
            </form>
          </div>
        ...
        <button @click="toggleTheme()" class="p-2 rounded-xl border flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2"
          ...
          <svg x-show="theme==='navy'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" /></svg>
          <svg x-show="theme!=='navy'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" /></svg>
        </button>
      </div>
    </header>
