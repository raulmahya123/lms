...
<a href="{{ $courseUrl }}" class="group soft-card lift flex h-full flex-col overflow-hidden rounded-3xl">
  <div class="relative aspect-[16/10] overflow-hidden bg-slate-100">
    ...
    <div class="absolute left-4 top-4 flex flex-wrap gap-2">
      @if ($popular)
        <span class="rounded-full bg-white px-3 py-1 text-xs font-black text-[#087A6C] shadow-sm">Populer</span>
      @endif
      <span class="rounded-full bg-slate-950/80 px-3 py-1 text-xs font-black text-white backdrop-blur">{{ $level }}</span>
    </div>
  </div>
  <div class="flex flex-1 flex-col p-6">
    <h3 class="line-clamp-2 text-lg font-black text-slate-950 group-hover:text-[#087A6C]">{{ $course->title }}</h3>
