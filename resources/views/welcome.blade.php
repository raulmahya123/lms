...
<style>
  ...
  .shine::after{
    content:"";position:absolute;inset:-100% -60% auto;height:160%;width:30%;
  ...
  .logo-chip{
    height:40px;width:40px;border-radius:9999px;background:#fff;display:flex;align-items:center;justify-content:center;
  ...
  .lock-badge{
    position:absolute;top:.75rem;right:.75rem;display:inline-flex;align-items:center;gap:.4rem;
  ...
  .divider{height:3px;width:68px;border-radius:999px;background:linear-gradient(90deg,#0284c7,#2563eb,#7c3aed)}
  ...
  .course-pill{
    position:absolute;top:.6rem;right:.6rem;z-index:10;display:inline-flex;align-items:center;gap:.35rem;padding:.35rem .55rem;
    border-radius:999px;font-size:.7rem;font-weight:700;background:#fff;color:#0f172a;border:1px solid rgba(2,6,23,.08);box-shadow:0 6px 20px rgba(2,6,23,.08)
  }
  .badge-level{font-size:.72rem;color:#1d4ed8;font-weight:600;opacity:.9}
  ...
  /* === PROFILE CARD (baru) === */
  .avatar-dot{display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:9999px;background:#e8f0ff;color:#1d4ed8;font-weight:700;border:1px solid rgba(2,6,23,.06)}
</style>
@endpush
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20 grid lg:grid-cols-2 gap-10 relative z-10">
    <div class="flex flex-col justify-center">
      ...
      <div class="mt-6 flex flex-col sm:flex-row gap-3">
        ...
        @guest
        <a href="{{ route('register') }}" class="hover-lift px-5 py-3 rounded-xl border text-center hover:bg-gray-50">
          <span class="inline-flex items-center gap-2">
            Daftar Gratis
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<section class="py-10 bg-gradient-to-b from-white to-sky-50/40">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid sm:grid-cols-3 gap-4">
      ...
      <div class="card p-5 hover-lift">
        <div class="flex items-center gap-2">
          <div class="h-9 w-9 rounded-xl bg-emerald-100 flex items-center justify-center text-emerald-700">🧩</div>
      ...
      <div class="card p-5 hover-lift">
        <div class="flex items-center gap-2">
          <div class="h-9 w-9 rounded-xl bg-indigo-100 flex items-center justify-center text-indigo-700">🎯</div>
            ...
            @if(!$isGuest && isset($course->progress_percent))
              <div class="mt-3">
                <div class="h-1.5 w-full bg-gray-200 rounded-full overflow-hidden">
                  <div class="h-full bg-gradient-to-r from-blue-600 to-indigo-600" style="width: {{ $pp }}%"></div>
                </div>
              ...
              <div class="p-5">
                ...
                <div class="mt-5 flex items-center gap-2">
                  @if($canAccessPsi)
                    ...
                    @auth
                      @php $last = collect($t->submissions ?? [])->where('user_id', auth()->id())->values()->last(); @endphp
                      @if($last)
                    ...
                    @endauth
                  @else
                    <a href="#plans" class="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg text-white
    ...
    <div class="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      @forelse($__threads as $t)
        <a href="{{ route('app.qa-threads.show', $t) }}"
           class="group card card-lg p-5 block hover-lift">
...
<section id="kupon" class="py-12 bg-white">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {{-- ALERT HASIL VALIDASI KUPON --}}
    <div id="alerts" class="mt-6">
      {{-- sukses --}}
      @if(session('coupon_status') === 'valid')
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">
          <div class="font-semibold">Kupon berhasil dipakai 🎉</div>
          <div class="mt-1 text-sm">
            @php
              $percent = session('discount_percent');
              $discAmt = session('discount_amount');
              $final   = session('final_amount');
            @endphp
            @if(!is_null($percent))
              Diskon: <strong>{{ $percent }}%</strong>
              @if(!is_null($discAmt))
            ...
            <strong>Rp {{ number_format((float) $final, 0, ',', '.') }}</strong>
          </div>
        </div>
      @endif

      {{-- gagal --}}
      @if(session('coupon_status') === 'invalid' || $errors->has('coupon'))
        <div class="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-800">
          <div class="font-semibold">Kupon gagal dipakai</div>
          <div class="mt-1 text-sm">
            {{ $errors->first('coupon') ?? 'Kupon tidak valid.' }}
          </div>
        </div>
      @endif
    </div>
    ...
    <div class="mt-6 grid md:grid-cols-3 gap-4">
      @forelse ($activeCoupons as $cp)
        <div class="card p-5 bg-gradient-to-br from-white to-sky-50 hover-lift">
          ...
          @auth
            <form method="POST" action="{{ route('app.coupons.validate') }}#alerts" class="mt-4">
              @csrf
              ...
              <button class="w-full px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 text-sm shine">
                Pakai Kupon
              </button>
            </form>
          @else
            <a href="{{ route('register') }}"
               class="mt-4 w-full inline-flex justify-center px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 text-sm shine">
